...
const mockPengeluaranData: PengeluaranData[] = [
  {
    id: '1',
    namaInvestasi: 'Pembangunan Ekowisata Pinus',
    tanggal: '01/01/2024',
    biayaPengeluaran: 5000000,
    dokumen: 'Pengeluaran.pdf',
  }
];

const BiayaPengeluaran: React.FC = () => {
  const [searchQuery, setSearchQuery] = useState('');

  const filteredData = mockPengeluaranData.filter((data) =>
            ...
            {filteredData.length > 0 ? (
              filteredData.map((data) => (
                ...
                >
                  ...
                  <td className="py-6 px-2 md:px-4 text-primary">
                    {formatRupiah(data.biayaPengeluaran)}
                  </td>
                  <td className="py-6 px-2 md:px-4 text-primary">
                    {/* Link Unduh Dokumen */}
                    <a 
              ...
              <tr>
                <td colSpan={4} className="py-12 text-center text-gray-400 font-medium text-sm">
                  Data biaya pengeluaran tidak ditemukan.
                </td>
              </tr>
            )}
          </tbody>
